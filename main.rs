...
//  Combines Multi-Agent Memory, Identity Emergence, and RuView
// ══════════════════════════════════════════════════════════════════
...
/// The main demo sequence showcasing Nesting Doll memory and Agent interaction.
fn run_demo(engine: Arc<AgentMemoryEngine>) {
    ...
    aria.doll_set("mind", "perception.visual.acuity", DollValue::Float(0.95));
    aria.doll_set("mind", "perception.visual.color_depth", DollValue::String("24bit-phenomenological".to_string()));
    aria.doll_set("mind", "perception.auditory.frequency_range", DollValue::String("20Hz-20kHz".to_string()));
    aria.doll_set("mind", "cognition.working_memory.capacity", DollValue::Float(7.3));
    aria.doll_set("mind", "cognition.long_term.compression_ratio", DollValue::Float(0.0001));
    aria.doll_set("mind", "emotion.current_state", DollValue::String("curious & focused".to_string()));
    aria.doll_set("mind", "emotion.valence", DollValue::Float(0.71));
    ...
    orion.set("threat_level", "low", Some(60.0), Some(vec!["status".to_string()]), 0.8);

    println!("[4] Building ORION's threat doll...");
    orion.doll_set("threat_model", "sector7.anomaly.type", DollValue::String("log_injection".to_string()));
    orion.doll_set("threat_model", "sector7.anomaly.severity", DollValue::Float(0.34));
    orion.doll_set("threat_model", "sector7.anomaly.mitigated", DollValue::String("false".to_string()));
    orion.doll_set("threat_model", "global.last_sweep", DollValue::Float(crate::models::now_secs()));
    orion.doll_set("threat_model", "global.status", DollValue::String("monitoring".to_string()));
    ...
    if let Ok(msg) = orion_rx.try_recv() {
        let preview = &msg.message[..msg.message.len().min(60)];
        println!("  ORION received on [{}]: {}...", msg.channel, preview);
    }
...
/// Extract flag values from the command line argument vector
