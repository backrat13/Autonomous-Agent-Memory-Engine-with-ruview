...
impl Cli {
    ...
    fn agent(&self) -> AgentProxy {
        self.engine.get_agent(&self.current_agent)
    }
    ...
    fn dispatch(&mut self, raw: &str) -> DispatchResult {
        ...
        match cmd.as_str() {
            ...
            "SET" => {
                ...
                a.set(key, val.as_str(), ttl, None, 1.0);
                println!("  OK");
            }
            ...
            "HSET" => {
                ...
                a.hset(&args[0], &args[1], args[2].as_str());
                println!("  OK");
            }
            ...
            "SADD" => {
                ...
                let added = if a.sadd(&args[0], args[1].as_str()) { 1 } else { 0 };
                println!("  added: {added}");
            }
            "SMEMBERS" => {
            ...
            "ZADD" => {
                ...
                match args[2].parse::<f64>() {
                    Ok(score) => { a.zadd(&args[0], args[1].as_str(), score); println!("  OK"); }
            ...
            // ── Snapshot ──────────────────────────────────────────
            "SNAPSHOT" => {
                let rdb_path = self.engine.config.rdb_path.clone();
                match self.engine.snapshot() {
                    Ok(sz) => println!("  ✓  {sz:>8} bytes → {rdb_path}"),
